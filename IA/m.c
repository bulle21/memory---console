...
void markAsDiscovered(int input1, int input2);  // Ajout du prototype manquant
...
void printTable(char *gameString, char *player1Inputs, char *machineInputs, int rows, int columns) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < columns; j++) {
            ...
            if (discoveredCells[cellIndex]) {
                ...
                // Texte bleu pour le joueur sans clignotement
                printf("   \x1b[34m%c\x1b[0m   │", gameString[cellIndex]);
            } else if ((int)machineInputs[cellIndex]) {
                // Texte bleu et clignotant pour la machine
    ...
    // Augmenter le temps d'affichage uniquement si c'est le tour de la machine
    if (turn == 2) {
        sleep(2); // Temps d'affichage prolongé pour le tour de la machine
    }
}
...
int checkTurn(char *gameString, int input1, int input2, int turn) {
    if (gameString[input1 - 1] == gameString[input2 - 1]) {
        ...
        // La machine rejoue si elle trouve une paire
        if (turn == 2) {
            machineReplays = 1;
        }
        return 1;
    }
    // Réinitialiser machineReplays si elle ne trouve pas de paire
    if (turn == 2) {
        machineReplays = 0;
    }
    return 0;
...
int machinePlay(char *gameString, int rows, int columns, int firstChoice) {
    // First, check if we can make a pair
    for (int i = 0; i < rows * columns; i++) {
    ...
    // If no pair found, choose a random undiscovered cell
...
int main() {
    ...
    system("clear");
    strcpy(name1,PLAYER);
    printf("\x1b[32mExcellent ! %s \x1b[0m\n\n",name1);
