    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < columns; j++) {
            ...
            if (discoveredCells[cellIndex]) {
                printf("       │");
            } else if ((int)player1Inputs[cellIndex] || (int)machineInputs[cellIndex]) {
                // Texte en bleu pour les cases retournées, avec alignement
...
void printGame(int turn, char *name1, char *name2, int points1, int points2) {
    ...
    turn == 1 ? printf("\t\tTour de : %s\n", name1) : printf("\t\t\x1b[36mTour de la Machine\x1b[0m\n");
